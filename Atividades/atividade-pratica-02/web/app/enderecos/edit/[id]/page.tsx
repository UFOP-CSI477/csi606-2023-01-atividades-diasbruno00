...
import { retornarEnderecosPorId } from '@/app/repository/enderecos/funcoes';

export default function EditarEnderecos({params}: any) {
  ...
  useEffect(() => {
    ...
    retornaTodosOsUsuarios().then(data => setUsuario(data))
      ...
      retornarEnderecosPorId(id).then(data => {
        console.log(data)
        setRua(data[0].rua)
  ...
  const handleSumit = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5555/update/endereco/${id}`, data)
