...
export default class CidadesDao {
     ...
     salvarCidadeNoBD(cidade ) {
        return new Promise((resolve, reject) =>  {
            conexao.query(
                'INSERT INTO CIDADES (nome,estadosId, criadoEm)  VALUES (?,?,?)', [cidade.nome, cidade.estadoId, cidade.criadoEm],
                (erro, resultado)  => {
                    if (erro) {
                        reject(erro);
                        return
     ...
     updateCidades(cidade, id)  {
        return new Promise((resolve, reject)=>  {
            conexao.query(
                'update estados set nome = ?, estadoId= ?, atualizadoEm = ? where id = ?', [cidade.nome, cidade.estadoId, cidade.atualizadoEm, id],
                (erro, resultado)  => {
                    if (erro) {
                        reject(erro)
                        return
